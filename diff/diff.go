...
// Heavily based on "A Formal Investigation of Diff3" by Sanjeev Khanna, Keshav Kunal,
// and Benjamin C. Pierce (https://www.cis.upenn.edu/~bcpierce/papers/diff3-short.pdf).
func Merge(textO, textA, textB string) string {
	dmp := diffmatchpatch.New()

	runesO, runesA, linesA := dmp.DiffLinesToRunes(textO, textA)
	_, runesB, linesB := dmp.DiffLinesToRunes(textO, textB)

	diffsA := dmp.DiffMainRunes(runesO, runesA, false)
	diffsB := dmp.DiffMainRunes(runesO, runesB, false)
...
// chunk merges the lines from o, a, and b int a single text.
